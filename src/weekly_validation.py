...
def main():
    ...
    # ---- Attach lon/lat from master (this fixes your KeyError) ----
    nodes_ll = pd.read_csv(
        args.master_csv,
        usecols=[c for c in ["node_id", "lat", "lon"]],
        low_memory=False,
    )
    nodes_ll["node_id"] = nodes_ll["node_id"].astype(str)
    nodes_ll = nodes_ll.dropna(subset=["lat", "lon"]).drop_duplicates("node_id")
    dfp = dfp.merge(nodes_ll, on="node_id", how="left")

    miss_ll = dfp["lat"].isna() | dfp["lon"].isna()
    if miss_ll.any():
        log.warning(
            f"[Join] Dropping {miss_ll.sum()} prediction rows without lat/lon after join."
        )
        dfp = dfp[~miss_ll].copy()
    ...
    for wk in weeks:
        ...
        node_week = (
            g_week[["node_id", "lat", "lon", pred_col]]
            .groupby(["node_id", "lat", "lon"], as_index=False)
            .mean()
        )

        # Master subset for this week
        ...
        for region_id, reg in REGIONS.items():
            ...
            axMap.set_xlim(reg["min_lon"], reg["max_lon"])
            axMap.set_ylim(reg["min_lat"], reg["max_lat"])
            ...
            if _CX_OK:
                ...
                axMap.scatter(
                    ...
                    cmap=aqi_category_cmap()[0],
                    norm=aqi_category_cmap()[1],
                    s=35,
                    edgecolors="black",
                    linewidths=0.3,
                    alpha=0.95,
